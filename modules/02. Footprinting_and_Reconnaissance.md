...
Recon-Dog:
> Recon-Dog is an all-in-one tool for all basic information gathering needs. It uses APIs to collect information about the target system. Censys, NS lookup, Port scan, Detect CMS, Whoislookup, Detect honeypot, Find subdomains, Reverse IP lookup, Detect technologies
...
- INFOGA
  > Infoga is a tool used for gathering email account information (IP, hostname, country, etc.) from different public sources (search engines, pgp key servers, and Shodan), and it checks
if an email was leaked using the haveibeenpwned.com API.
- eMailTrackerPro
  > Attackers use eMailTrackerPro to analyze email headers and extract information such as the sender's geographical location, IP address, and so on. It
